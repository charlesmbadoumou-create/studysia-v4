...
async function activateAumOnly() {
  await initDb();
  await run("UPDATE institutions SET active_etablissement = 0");
  await run("UPDATE programs SET active_formation = 0");

  await run(
    "UPDATE institutions SET active_etablissement = 1 WHERE LOWER(name) LIKE '%aum%' OR LOWER(handle) LIKE '@aum_%'"
  );
  ...
  console.log("- Active institutions: AUM only");
